...
This guide provides step-by-step instructions to set up the SP1 and RISC Zero provers f:::tip
...
You can find `SP1_VERIFIER_ADDRESS` and `GROTH16_VERIFIER_ADDRESS` addresses 
in the output of the `surge-protocol-deployer.sh` script under `Extracting Surge L1 SCs deployment results...` section. 
`SP1_VERIFIER_ADDRESS` is called `SUCCINCT_VERIFIER` and `GROTH16_VERIFIER_ADDRESS` is called `RISC0_GROTH16_VERIFIER`.

For `SP1_VERIFIER_RPC_URL` and `GROTH16_VERIFIER_RPC_URL`, you can use the same L1 RPC URL that you used while deploying the protocol.
...
:::tip
You could leave `RAIKO_CONF_DIR`, `BASE_CONFIG_FILE` and `BASE_CHAINSPEC_FILE` as default if you have placed your config files in `raiko/host/config/devnet`.

You could find `SP1_VERIFIER_ADDRESS` and `GROTH16_VERIFIER_ADDRESS` addresses 
in the output of the `surge-protocol-deployer.sh` script under `Extracting Surge L1 SCs deployment results...` variable 
`SP1_VERIFIER_ADDRESS` is called `SUCCINCT_VERIFIER` and `GROTH16_VERIFIER_ADDRESS` is called `RISC0_GROTH16_VERIFIER`.

For `SP1_VERIFIER_RPC_URL` and `GROTH16_VERIFIER_RPC_URL`, you could use the same L1 RPC URL that you used while deploying the protocol.
