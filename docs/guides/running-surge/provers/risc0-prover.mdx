...
Bento is a proving server designed for RISC Zero applications. It handles proof generation requests and provides a dedicated service for ZK proof computation. The Raiko prover will request proofs from Bento to generate the necessary cryptographic proofs for the rollup. Learn more at the [RISC0 Bento repository](https://github.com/NethermindEth/risc0-bento/tree/main).
...
2. Copy the sample environment file:
    ```bash
    cp bento/dockerfiles/sample.env ./
    ```
...
Run the following command:
```bash
cargo build -F cuda --release --features risc0 
```
...
export BONSAI_API_URL=http://0.0.0.0:8081 # Your Bonsai (Bento) API URL
```
