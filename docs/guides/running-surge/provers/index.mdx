...
For two-VM, paste [`deploy-real-zisk-two-vm.md`](https://github.com/NethermindEth/simple-surge-node/blob/main/agent-prompts/deploy-real-zisk-two-vm.md) into LLM model as a subagent. For same-VM use [`deploy-real-zisk-same-vm.md`](https://github.com/NethermindEth/simple-surge-node/blob/main/agent-prompts/deploy-real-zisk-same-vm.md). Both have privacy mode as a toggle and bake in the ~16 min single-GPU cold-start waits so the agent doesn't kill/retry.
:::
...
raiko is now a submodule of [simple-surge-node](https://github.com/NethermindEth/simple-surge-node), so the prover-side and L2-side scripts ship from the same repo. Clone once and use `deploy-prover.sh` to set everything up:
...
1. NVIDIA + Docker pre-flight (exits cleanly if no GPU; recommends `--mock-prover` instead)
2. `raiko/script/install-zisk-deps.sh` — apt deps, Rust, CUDA 12.x; detects/purges legacy CUDA 11.x leftovers
3. `TARGET=zisk make install` inside raiko — downloads ZisK proving keys to `~/.zisk` (~150 GB; override with `ZISK_DIR=/path/to/large/disk` in the env)
4. `cp docker/.env.sample.zk docker/.env` if not present
5. `docker compose -f docker/docker-compose-zk.yml up -d`
6. Polls `http://localhost:8080/guest_data` until the vkey returns (timeout 30 min)

Expected vkey response when ready:

```json
{"zisk":{"batch_vkey":"2ccafa601f5e29e4d61fc2d96474c98c3b752999b97af38c4c988c0fee24f0a0"}}
```

For privacy mode add `--privacy-mode` (generates a bundle locally and rebuilds guest ELFs) — see Step 5.
...
:::tip Re-runnable
`deploy-prover.sh` is idempotent. Pass `--skip-install` and/or `--skip-zisk-sdk` to skip the heavy installs on re-runs (the script auto-detects an existing `~/.zisk/provingKey` and skips that one anyway).
:::
...
### Step 5 — Privacy mode (optional)

Privacy mode encrypts blob payloads end-to-end. It's orthogonal to the prover type, but the real ZisK guest needs key hashes baked in at compile time — the helper script (`./script/sync-privacy-to-prover.sh --rebuild user@<prover-host>`) does the scp + rebuild + recreate in one shot. See the full **[Privacy Mode](../privacy-mode.mdx)** guide for the workflow, key-rotation procedure, and troubleshooting.
