...
In Surge, every proposed block needs to be proven. To ensure a 1-to-1 mapping from proposals to proofs, 
Surge requires proposers to provide collateral; the proving liveness bond, committing to submitting 
the proof within the proving liveness window (currently set to 24 hours), or forfeitting the the proving liveness bond. 
In the event that the original prover does not submit the proof in time, any prover can instead submit a proof for the block, and collect the collateral. 
As such, the proving liveness bond acts as both an incentive for the original prover to submit the proof, and as an incentive for another prover to provide the proof in case of original prover failure.
...
- the cost to run all 3 proof systems, in the worst case scenario where there is a bug in 1 of the 3 proof systems-

On mainnet, we can use mainnet ETH as the collateral, creating a clear financial incentive for provers.
On testnet however, no financial incentive for provers exists. To handle this, we have chosen the proving liveness bond to be 10,000 testnet ETH. 
This number is chosen to balance permissionlessness and spam protection for Surge.
...
- **Spam protection**: If a block proposer does not provide a proof within the proving liveness window, a malicious block, the proposer will lose the 10,000 testnet ETH proving liveness bond. This creates a loose upperbound on the number of malicious blocks that can be proposed on Surge. Such an upperbound gives any public good prover operating on testnet a chance to eventually prove these blocks, ensuring the queue of unproven blocks on Surge never becomes too long. In turn, this ensures two critical properties that we must ensure for a rollup:
