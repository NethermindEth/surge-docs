...
:::note
You **can** skip deploying provers in a local devnet if you only want to test basic transaction flow and block proposals. However, because Surge uses a **2-of-4 multi-prover security model**, without **at least two** provers running:
...
### What is a Prover?
A prover is a component responsible for generating cryptographic proofs that validate the correctness and integrity of Layer 2 transactions and blocks. Surge leverages multiple provers — four independent ones in total — to ensure the security and reliability of rollup chains.
...
- Surge's **2-of-4 model** means that having two of these provers independently confirm a block is sufficient to finalize it on-chain.

### How Do Provers Work?
Provers execute computationally intensive algorithms to create succinct proofs that validate batches of L2 transactions. Once generated, these proofs are submitted on-chain via a prover-relayer, which then finalizes and confirms the validity of these transactions on the blockchain—provided at least two of the four available provers agree on the block.

## Prover Requirements for Surge

### Surge Proof Systems

Surge supports four provers in production scenarios:

- [SGX](sgx-prover.mdx)
- [SGXGETH](sgx-prover.mdx)
- [RISC0](sp1-and-risc0-provers.mdx)
- [SP1](sp1-and-risc0-provers.mdx)

Although you only need **two** of these provers to finalize blocks under Surge’s 2-of-4 security model, you may choose to run all four for maximum redundancy and security. Each prover has distinct setup and operational requirements. Follow the respective guides carefully to properly configure, deploy, and maintain each.
...
- the cost to run all 4 provers, in the worst case scenario where there is a bug in 1 of the 4 provers.
