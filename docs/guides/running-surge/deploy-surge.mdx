...
After the L2 stack is up but **before** any L2 transaction (DEX deployment is the first one), the script verifies Raiko is reachable.
...
- **Real prover** — Raiko runs on a separate machine. The check uses `$RAIKO_HOST_ZKVM` (transforming `host.docker.internal` → `localhost` for host-side reachability). It runs three sequential probes against the prover URL:
  1. `GET /health` (basic liveness, must return 200)
  2. `GET /guest_data` (must return a `zisk.batch_vkey` — first call takes 4–5 min on cold start)
...
The real-prover check has a 30-minute timeout to cover single-GPU first-cold-start scenarios. If Raiko isn't ready in that window, the script aborts before any L2 tx hits Catalyst — no premature proof requests, no L2 reorgs.
