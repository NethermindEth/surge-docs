...
| 7 | Test token on L1? | `0` yes / `1` no |
| 8 | Test token on L2? | `0` yes / `1` no |
...
`ProofVerifierDummy` requires proofs signed by `MOCK_PROOF_SIGNER`. Confirm `MOCK_PROOF_MODE=true` was set during L1 deployment and that the key in `.env` matches.
