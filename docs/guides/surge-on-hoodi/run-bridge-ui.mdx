...
Checkout the `surge/hoodi` branch:

```bash
git checkout surge/hoodi
```
...
Use Docker Compose to run the Bridge UI and nginx services:
...
:::warning
Double-check that all URLs in your nginx config are secured and publicly accessible before running.
:::
